import React from "react";
import topImage from "../../assets/projects-top.png";
import technology from "../../assets/pacific-biochar-agriculture.avif";
import enhanced_mineral from "../../assets/future-forest-stream.avif";
import factory from "../../assets/ascend_decatur.avif";
import refrigerant from "../../assets/tradewater-gas-destruction-warehouse.avif";
import sap from "../../assets/cot-saplings3.avif";
import air_task from "../../assets/clean-air-task-force.avif";
import carbon from "../../assets/carbon180.avif";
import amazon from "../../assets/peruvian-rainforest-panoramic.avif";
import mangroove from "../../assets/mangrove-leaves2.avif";
const Section3 = () => {
  return (
    <>
      <div className='four-img' id='projects'>
        <img src={topImage} alt='' />
        <h1>
          Forzero members fund 4 types of <br />
          climate solutions
        </h1>
        <svg
          fill='#cccccc'
          height='82'
          viewBox='0 0 15 82'
          width='15'
          xmlns='http://www.w3.org/2000/svg'
          className='arrow'
        >
          <path d='M5.50391 0H9.49992V74H5.50391L5.50391 0Z'></path>
          <path d='M7.5 81.5005L1.60935e-06 74.0005L15 74.0005L7.5 81.5005Z'></path>
        </svg>
      </div>
      <div className='all-projects-inner font-Blanco'>
        <div className='upper-header'>
          <div className='top-image'>
            <svg
              viewBox='-1.5 0 15 15'
              fill='#F79F22'
              xmlns='http://www.w3.org/2000/svg'
            >
              <path d='M11.525 10.17 8.048 4.663a1.338 1.338 0 0 1-.206-.714V1.793h.433c.24 0 .445-.185.457-.424a.445.445 0 0 0-.445-.467h-4.44a.454.454 0 0 0-.458.424.445.445 0 0 0 .445.466h.445V3.95c0 .252-.07.5-.206.713L.597 10.17a2.06 2.06 0 0 0-.07 2.11c.37.679 1.09 1.09 1.863 1.09h7.341c.773 0 1.494-.411 1.863-1.09a2.06 2.06 0 0 0-.07-2.11ZM4.827 5.138c.224-.356.343-.768.343-1.189V1.904a.111.111 0 0 1 .111-.111H6.84a.111.111 0 0 1 .111.11V3.95c0 .42.12.833.344 1.189l1.609 2.547a.223.223 0 0 1-.189.342H3.406a.223.223 0 0 1-.188-.342l1.609-2.547Z'></path>
            </svg>
          </div>
          <div className='header-txt'>
            <h1>1. Technology</h1>
            <p>
              Carbon removal technologies like biochar and mineral weathering
              turn atmospheric CO2 into solid rock—locking up carbon for
              thousands of years.
            </p>
          </div>
          <div className='inner'>
            <div className='inner-body'>
              <h1 className='inner-h1'>Biochar in California</h1>
              <div className='body-img'>
                <img src={technology} alt='' />
              </div>
              <div className='body-txt'>
                <h1>Biochar in California</h1>
                <h2>
                  This project helps prevent wildfires in California’s
                  old-growth forests by removing dead and flammable trees. They
                  then use a cutting-edge process to turn the tree biomass into
                  biochar — keeping carbon out of the air for thousands of
                  years.
                </h2>
                <div className='inner-body-txt'>
                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Prevents wildfires</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Permanent carbon storage</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Based in California</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Promotes biodiversity</div>
                  </div>
                </div>
              </div>
            </div>

            <div className='inner-body rev'>
              <h1 className='inner-h1'>Enhanced Mineral Weathering</h1>
              <div className='body-img'>
                <img src={enhanced_mineral} alt='' />
              </div>
              <div className='body-txt rev-txt'>
                <h1>Enhanced Mineral Weathering</h1>
                <h2>
                  This project adds crushed basalt to forest land, speeding up
                  Earth's natural carbon cycle– and sequestering carbon for
                  millions of years.
                </h2>
                <div className='inner-body-txt'>
                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Supports novel climate research</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Permanently stores carbon in rock</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Fast-growing project with potential to scale</div>
                  </div>
                </div>
              </div>
            </div>

            <div className='inner-body '>
              <h1 className='inner-h1'> Adipic acid nitrous oxide abatement</h1>
              <div className='body-img'>
                <img src={factory} alt='' />
              </div>
              <div className='body-txt '>
                <h1>Adipic acid nitrous oxide abatement</h1>
                <h2>
                  This project reduced nitrous oxide (N₂O) emissions from Ascend
                  Performance Materials’ chemical plant in Cantonment, Florida.
                  Nitrous oxide is an often-forgotten climate supervillain —
                  it’s 265 times more harmful than CO₂ and in 2021 was
                  responsible for 6% of human-generated greenhouse gas emissions
                  in the U.S.
                </h2>
                <div className='inner-body-txt'>
                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Captures nitrous oxide emissions</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Highly measurable</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Low cost</div>
                  </div>
                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Readily deployable technology</div>
                  </div>
                </div>
              </div>
            </div>

            <div className='inner-body rev'>
              <h1 className='inner-h1'>Refrigerant destruction </h1>
              <div className='body-img'>
                <img src={refrigerant} alt='' />
              </div>
              <div className='body-txt rev-txt'>
                <h1>Refrigerant destruction </h1>
                <h2>
                  This project permanently destroys harmful refrigerants that
                  would otherwise be released into the atmosphere—thereby
                  fighting global warming, and helping to restore the planet's
                  ozone layer.
                </h2>
                <div className='inner-body-txt '>
                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Prevents ozone depletion</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Highly measurable</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Low cost</div>
                  </div>
                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Creates green jobs</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div className='upper-header'>
          <div className='top-image top-2'>
            <svg
              fill='#27ae60'
              viewBox='-2.5 0 15 15'
              xmlns='http://www.w3.org/2000/svg'
            >
              <path d='M0.432401 12.0864L2.4629 8.58307H1.61924C1.56205 8.58307 1.50485 8.55447 1.47625 8.49728C1.44765 8.45438 1.44765 8.38288 1.47625 8.33998L3.27797 5.33712H2.69169C2.6488 5.33712 2.6202 5.30853 2.5916 5.27993C2.5773 5.25133 2.5773 5.20843 2.5916 5.16553L4.97959 1.03303C4.99389 1.00443 5.03678 0.97583 5.07968 0.97583C5.12258 0.97583 5.16548 0.990129 5.17978 1.03303L7.56777 5.17983C7.58207 5.20843 7.58207 5.25133 7.56777 5.29423C7.55347 5.32283 7.51057 5.35142 7.46767 5.35142H6.8814L8.68311 8.35428C8.71171 8.39718 8.71171 8.46868 8.68311 8.51158C8.65451 8.55447 8.59732 8.59737 8.54012 8.59737H7.69646L9.74126 12.0864C9.78416 12.1579 9.78416 12.2437 9.74126 12.3152C9.69837 12.3867 9.62687 12.4296 9.55537 12.4296H5.70885V14.4029C5.70885 14.4601 5.65166 14.5173 5.59446 14.5173H4.57921C4.52201 14.5173 4.46481 14.4601 4.46481 14.4029V12.4153H0.618293C0.532497 12.4153 0.460999 12.3724 0.432401 12.3009C0.389503 12.2294 0.389503 12.1436 0.432401 12.0864Z'></path>
            </svg>
          </div>
          <div className='header-txt'>
            <h1>2. Tree Planting</h1>
            <p>
              Trees are nature’s soldiers in the fight against climate change,
              and Forzero members help plant millions of native species every
              year.
            </p>
          </div>
          <div className='inner'>
            <div className='inner-body '>
              <h1 className='inner-h1'>Community agroforestry </h1>
              <div className='body-img'>
                <img src={sap} alt='' />
              </div>
              <div className='body-txt'>
                <h1>Community agroforestry</h1>
                <h2>
                  This project empowers smallholder farmers in Timor-Leste to
                  restore degraded ecosystems through reforestation and improved
                  land management.
                </h2>
                <div className='inner-body-txt'>
                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Promotes biodiversity</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Provides economic opportunity</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Improves farmland</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div className='upper-header'>
          <div className='top-image top-3'>
            <svg
              fill='#5B27F0'
              viewBox='-0.5 -1 15 15'
              xmlns='http://www.w3.org/2000/svg'
            >
              <path d='M13.52 5.865h-.156l-1.538-3.197a.709.709 0 1 0-1.06-.914l-.653-.507A2.645 2.645 0 0 0 7.928.754a.708.708 0 0 0-1.401 0 2.645 2.645 0 0 0-2.19.497l-.652.507a.708.708 0 1 0-1.06.914L1.09 5.865H.935a.182.182 0 0 0-.18.196 2.34 2.34 0 0 0 4.662 0 .182.182 0 0 0-.18-.196h-.154L3.54 2.668l.013-.012.823-.503a1.966 1.966 0 0 1 1.792-.13c.085.03.173.055.261.073v7.711l-3.317 1.054a.522.522 0 0 0-.369.503v.034a.522.522 0 0 0 .523.522h7.915a.522.522 0 0 0 .522-.522v-.04a.523.523 0 0 0-.363-.497L8.027 9.817V2.093a1.75 1.75 0 0 0 .261-.073 1.967 1.967 0 0 1 1.793.13l.823.503.013.012-1.546 3.2h-.152a.182.182 0 0 0-.18.196 2.34 2.34 0 0 0 4.662 0 .182.182 0 0 0-.182-.196Zm-9.198 0H1.84l1.238-2.568 1.243 2.568Zm5.809 0 1.238-2.568 1.237 2.568h-2.475Z'></path>
            </svg>
          </div>
          <div className='header-txt'>
            <h1>3. Policy</h1>
            <p>
              Forzero members support leading policy groups that push for
              changes to our economies, agriculture, infrastructure, power
              grid—every system that influences our planet’s climate.
            </p>
          </div>
          <div className='inner'>
            <div className='inner-body '>
              <h1 className='inner-h1'>Clean Air Task Force</h1>
              <div className='body-img'>
                <img src={air_task} alt='' />
              </div>
              <div className='body-txt '>
                <h1>Clean Air Task Force</h1>
                <h2>
                  This organization is a policy group that advocates for new
                  technologies and policies needed to get to a zero-emissions,
                  high-energy planet at an affordable cost.
                </h2>
                <div className='inner-body-txt'>
                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Influences politicians on climate policy</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Promotes key climate solutions</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Top-rated climate charity</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>25+ years of success</div>
                  </div>
                </div>
              </div>
            </div>

            <div className='inner-body rev '>
              <h1 className='inner-h1'>Carbon180</h1>
              <div className='body-img'>
                <img src={carbon} alt='' />
              </div>
              <div className='body-txt rev-txt'>
                <h1>Carbon180</h1>
                <h2>
                  This organization helps improve the transparency and
                  scientific integrity of carbon removal and climate solutions
                  through open data, tools, and leadership.
                </h2>
                <div className='inner-body-txt'>
                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Promotes better climate policy</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Tracks carbon removal legislation</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Promotes carbon-removal on U.S. farmland</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className='upper-header'>
          <div className='top-image top-4'>
            <svg
              fill='#1269d3'
              viewBox='-1 0 16 16'
              xmlns='http://www.w3.org/2000/svg'
            >
              <path d='M11.816 10.036c-.865.732-2.056.755-2.864.655a6.681 6.681 0 0 1 2.019-1.414c2.939-1.303 2.67-5.422 2.653-5.468-.026-.73-1.205-.698-1.153.087 0 .029.175 2.693-1.353 3.948a3.423 3.423 0 0 0-.963-2.307 6.902 6.902 0 0 0 1.442-2.691.579.579 0 0 0-1.12-.289 5.752 5.752 0 0 1-1.179 2.215c-.865-1.052-.303-3.057-.288-3.08a.577.577 0 0 0-1.105-.329c-.04.133-.963 3.303 1.096 4.678 1.079.719 1.033 1.939.96 2.446a8.046 8.046 0 0 0-2.324 1.938 8.199 8.199 0 0 0-.289-1.551 4.802 4.802 0 0 0 .796-1.73.58.58 0 1 0-1.136-.229 3.98 3.98 0 0 1-.236.67c-.545-.9-.866-.92-1.587-1.748a5.062 5.062 0 0 0 1.246-3.424.577.577 0 0 0-.675-.577c-.917.153-.017 1.41-1.26 3.032a4.993 4.993 0 0 1-.66-1.662.577.577 0 0 0-.652-.504c-1.194.141.072 2.506.75 3.417.617 1.235 2.526 1.301 2.564 5.056A12.914 12.914 0 0 0 3.59 9.27c.326-.387.514-.871.534-1.376a.577.577 0 1 0-1.154.023c-.041.298-.18.574-.395.785C1.188 7.53 1.528 5.297 1.528 3.855a.577.577 0 0 0-.767-.548c-.464.147-.386.652-.386 1.125 0 1.485-.24 4.222 1.975 5.509.346.23 1.301.513 2.677 1.531.741.551.383 1.333.176 2.019a.577.577 0 0 0 .553.747h2.487a.577.577 0 0 0 .553-.741l-.392-1.3a.289.289 0 0 1 .317-.37c1.341.184 2.74 0 3.799-.865a.581.581 0 1 0-.704-.926Z'></path>
            </svg>
          </div>
          <div className='header-txt'>
            <h1>4. Conservation</h1>
            <p>
              Natural ecosystems like old growth forest, peat bogs, and mangrove
              swamps are the lungs of the earth. Wren members help protect them.
            </p>
          </div>
          <div className='inner'>
            <div className='inner-body '>
              <h1 className='inner-h1'>Amazon Rainforest protection</h1>
              <div className='body-img'>
                <img src={amazon} alt='' />
              </div>
              <div className='body-txt '>
                <h1>Amazon Rainforest protection</h1>
                <h2>
                  This project gives Indigenous Amazonians the tools they need
                  to protect their rainforest home. Using satellite monitoring
                  and drones, Indigenous monitors detect deforestation early.
                  This allows communities to prevent deforestation before it
                  reaches a large scale.
                </h2>
                <div className='inner-body-txt'>
                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Protects biodiversity</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Prevents deforestation</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Promotes Indigenous land rights</div>
                  </div>
                </div>
              </div>
            </div>

            <div className='inner-body rev '>
              <h1 className='inner-h1'>Mangrove conservation in Mexico</h1>
              <div className='body-img'>
                <img src={mangroove} alt='' />
              </div>
              <div className='body-txt rev-txt'>
                <h1>Mangrove conservation in Mexico</h1>
                <h2>
                  This project protects the mangrove forests of Baja California
                  Sur, Mexico by scaling-up conservation efforts. In addition to
                  their carbon-storing abilities, mangroves provide a wide range
                  of ecosystem services, including shoreline protection, water
                  filtration, and habitat for critical marine species.
                </h2>
                <div className='inner-body-txt'>
                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Protects biodiversity</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Preserves mangroves</div>
                  </div>

                  <div className='tick-body'>
                    <div className='tick-mark'>
                      <svg
                        id='Layer_1'
                        data-name='Layer 1'
                        xmlns='http://www.w3.org/2000/svg'
                        viewBox='0 0 40 40'
                        width='20'
                        height='20'
                      >
                        <path
                          d='M39.2929 10.7071C39.6834 10.3166 39.6834 9.68345 39.2929 9.29293L35.7071 5.70711C35.3165 5.31658 34.6834 5.31658 34.2929 5.70711L15.7071 24.293C15.3166 24.6836 14.6834 24.6836 14.2929 24.293L5.70714 15.7072C5.31662 15.3167 4.68345 15.3167 4.29292 15.7072L0.70711 19.293C0.316585 19.6835 0.316586 20.3167 0.707114 20.7072L14.2929 34.2928C14.6834 34.6833 15.3165 34.6833 15.7071 34.2928L39.2929 10.7071Z'
                          fill='var(--wren-orange)'
                        ></path>
                      </svg>
                    </div>
                    <div>Provides community benefits</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Section3;
